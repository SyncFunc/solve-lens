...
export function ProviderCard({ config, setConfig, threadId }: { config: Config; setConfig: React.Dispatch<React.SetStateAction<Config>>; threadId?: string }) {
  ...
  return <Card className="panel-card" title={<span><IconSettings /> Provider 与模型</span>}>
    ...
    <div className="provider-hint">{config.provider === "openai-api" ? "OpenAI API 按单轮请求处理，不维护 Codex 会话上下文。" : `已读取 ${models.length || "当前"} 个 CLI 模型；当前模型：${modelLabel}。${config.codex_execution_mode === "interactive" ? "交互模式会保留 Codex thread，上下文仅在清空时归档。" : "单题模式每次独立执行。"} Fast 使用 service_tier=priority，仅在模型/账号提供该层时可用。`}</div>
    <div className="current-value"><Tag color="green">当前生效 Provider：{config.provider === "codex-cli" ? "Codex CLI" : config.provider === "openai-api" ? "OpenAI API" : "Claude Code CLI"}</Tag><Tag color="arcoblue">当前模型：{config.provider === "openai-api" ? config.openai_model : config.codex_model || modelLabel}</Tag>{config.provider === "codex-cli" ? <><Tag color="arcoblue">推理：{config.codex_reasoning_effort || `模型默认（${currentReasoning}）`}</Tag><Tag color={serviceTier === "priority" ? "orange" : "gray"}>速度：{serviceTier === "priority" ? "Fast · 约 1.5×" : "标准 · 1×"}</Tag></> : null}</div>
  </Card>;
}
